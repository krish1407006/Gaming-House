...
export const getAdminTrending = async (req, res) => {
  try {
    const result = await gameService.getAdminTrendingGames();
    ...
    console.error("Error in admin getAdminTrending:", error);
    res.status(500).json({ error: "Internal server error" });
  }
};

export const reorderTrending = async (req, res) => {
  try {
    ...
    const result = await gameService.reorderTrending(orderedIds);
    ...
    console.error("Error in admin reorderTrending:", error);
    res.status(500).json({ error: "Internal server error" });
  }
};
